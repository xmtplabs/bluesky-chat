...
/**
 * Resolve an array of user profiles to XMTP inbox IDs.
...
export async function resolveUsersToInboxIds(
  ...
  for (const user of users) {
    const inboxId = await identityService.resolveIdToInboxCached(user.id)
    if (inboxId) {
      identityService.cacheMapping(inboxId, user.id)
